...
import React, { useState, useEffect, type ChangeEvent, type FormEvent, useRef } from 'react'; // PERBAIKAN: Menggunakan 'type' untuk import
import { type Skill, addHardSkills, fetchMasterHardSkills, skillLevels } from '../../services/SkillsService';
import Swal from 'sweetalert2';
import { Save, X, PlusCircle, Zap } from 'lucide-react';
...
const AddHardSkillsForm: React.FC<AddHardSkillsFormProps> = ({ onClose, onAddSuccess, existingSkillNames }) => {
  ...
  const formRef = useRef<HTMLFormElement>(null); // Ref untuk keseluruhan form
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (skillsToSubmit.length === 0) {
      Swal.fire('Input Kosong', 'Mohon isi setidaknya satu hard skill.', 'warning');
      return;
    }
    ...
    if (newSkillsPayload.length !== skillsToSubmit.length) {
        Swal.fire('Skill Duplikat', 'Beberapa skill yang Anda masukkan sudah ada dan tidak akan ditambahkan lagi.', 'info');
    }
    ...
    try {
      ...
      Swal.fire('Sukses!', 'Hard skill berhasil ditambahkan.', 'success');
      onAddSuccess();
      onClose();
    } catch (err: any) {
      Swal.fire('Error', err.message || 'Gagal menambahkan hard skill.', 'error');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex justify-center items-center z-[100] p-4">
      <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-lg max-h-[95vh] flex flex-col">
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800 flex items-center"><Zap size={24} className="mr-2 text-orange-500" /> Tambah Hard Skill</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors p-1 rounded-full hover:bg-gray-100">
            <X size={24} />
          </button>
        </div>

        <form ref={formRef} onSubmit={handleSubmit} className="flex-grow overflow-y-auto pr-2 space-y-4">
          {skillsToAdd.map((skill, index) => (
            <div key={index} className="p-3 border border-gray-200 rounded-lg space-y-3 bg-gray-50/50">
              <div className="relative">
                <label htmlFor={`hardskill-name-${index}`} className="block text-xs font-medium text-gray-700 mb-1">Nama Hard Skill</label>
                <input
                  type="text"
                ...
                {showDropdowns[index] && (
                  <ul className="absolute z-20 w-full bg-white border border-gray-300 rounded-md mt-1 max-h-40 overflow-y-auto shadow-lg">
                    {isLoadingMasterSkills ? ( <li className="px-3 py-2 text-sm text-gray-400">Memuat...</li> ) : 
                    getFilteredMasterSkills(index).length > 0 ? (
                      getFilteredMasterSkills(index).map(ms => (
                        <li key={ms.id} onClick={() => handleSelectMasterSkill(index, ms.name)} className="px-3 py-2 text-sm hover:bg-blue-50 cursor-pointer">{ms.name}</li>
                      ))
                    ) : (
                      <li className="px-3 py-2 text-sm text-gray-500">"{searchTerms[index]}" akan ditambahkan sebagai skill baru.</li>
                    )}
                  </ul>
                )}
              </div>
              <div>
                <label htmlFor={`hardskill-level-${index}`} className="block text-xs font-medium text-gray-700 mb-1">Level Kemahiran</label>
                <select id={`hardskill-level-${index}`} value={skill.level} onChange={(e) => handleSkillLevelChange(index, e.target.value as Skill['level'])} className="w-full px-3 py-2 text-sm rounded-md border border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 appearance-none bg-white" disabled={isSaving}>
                  {skillLevels.map(level => <option key={level} value={level}>{level}</option>)}
                </select>
              </div>
              {skillsToAdd.length > 1 && <button type="button" onClick={() => removeSkillField(index)} className="text-xs text-red-500 hover:text-red-700">Hapus Skill Ini</button>}
            </div>
          ))}
          <button type="button" onClick={addSkillField} className="mt-2 text-sm text-blue-600 hover:text-blue-800 flex items-center" disabled={isSaving}>
            <PlusCircle size={16} className="mr-1" /> Tambah Hard Skill Lain
          </button>
        </form>
