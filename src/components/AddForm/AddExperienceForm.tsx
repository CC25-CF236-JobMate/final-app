...
const AddExperienceForm: React.FC<AddExperienceFormProps> = ({ onClose, onAddSuccess }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      Swal.fire('Sukses!', 'Pengalaman kerja berhasil ditambahkan.', 'success');
      onAddSuccess(newExperience);
      onClose();
    } catch (err: any) {
      console.error("Failed to add experience:", err);
      Swal.fire('Error', err.message || 'Gagal menambahkan pengalaman kerja.', 'error');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex justify-center items-center z-[100] p-4">
      <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-lg max-h-[95vh] flex flex-col">
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          <h2 className="text-xl sm:text-2xl font-semibold text-gray-800">Tambah Pengalaman Kerja</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto pr-2 flex-grow custom-scrollbar-modal">
          {inputFields.map(field => (
            <div key={field.name}>
              <label htmlFor={`add-exp-${field.name}`} className="block text-xs font-medium text-gray-700 mb-1 flex items-center">
